...
\begin{itemize}[<+->]
    \item Sources of (psuedo)randomness are essential to cryptography libraries.
    \item They also enable the class of randomized algorithms (albeit costly).
...
\begin{itemize}[<+->]
    ...
    Something like SHA-256, but that requires mod-$2^{32}$ addition which is tricky whether we encode inputs as ints or bitstrings. 
    \item Cipher-based: either block or stream ciphers, generating a sequence of pseudorandom outputs.
    AES has the SubBytes step which is a byte-substitution lookup table (8 depth for a polynomial that evaluates the LUT), and ChaCha also requires mod-$2^{32}$ addition so neither are particularly convenient to implement.
\end{itemize}
\end{frame}
...
rngeo uses a Rasta-like PRF implementation on a 256-bit input; the same substitution layer as Rasta, but has a different affine layer (Rasta's uses a matrix multiplication).
...
\begin{frame}
\frametitle{Potential work directions}
\begin{itemize}
    \item Implement one of the common PRFs: involves coding up the missing components (eg. 32-bit adders or AES' SubBytes LUT)
    \item Optimization of existing circuit: how can we make the current implementation(s) faster, lower depth, etc?
    \item Search for other potential PRFs: what other PRFs can potentially be used within FHE?
\end{itemize}
\end{frame}
